...
def genChart(f,variant,tabid):
 ...
 #   f.write('<li><span class="family">Google Scholar Citation</span> ('+ row["Timestamp"] +'):   '+ str(row["Citation count (google scholar)"]) + '</li>\n')
    ...
    f.write('<li><span class="family">Time spent for the test (build->first run, timeout at 100min)</span>: '+ str(variant["Time spent for the test (code download to first successful run)"]*10) + 'min</li>\n')
